...
class AccountPool(object):
    ...
    def count_usable(self, realm=None):
        """How many accounts could actually serve a request right now.

        ``count_ready()`` only checks the enabled flag and that a token
        exists, so an account sitting in cooldown or holding an expired
        token with no way to refresh still counts. That makes the panel
        report capacity the pool cannot deliver, and leaves users staring
        at "2/2 usable" next to a 503.

        This walks the same tests ``pick()`` applies. It deliberately does
        not call ``Account.ready()``: that method may hit the network to
        refresh an expiring token, which is far too slow for a status read
        that the dashboard polls on a timer.
        """
        now = time.time()
        with self._lock:
            snapshot = [a for a in self.accounts if not realm or a.realm == realm]
